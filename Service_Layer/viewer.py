...
#        print(instances)
    ...
    def print_instances():
        ...
        for instance_name in instances:
            ...
            lfr = round(i.flangeability_parameters.overall_lfr, 2)
            if her==0:
                her=''
...
#            print('%s: d0 = %.1f mm, R = %d mm' % (instance_name, d0, R))
    ...
    def print_status(instance_name):
        ...
        if next_action > 6:
            ...
            overall_lfr = i.flangeability_parameters.overall_lfr
            lfr_per_tool = i.flangeability_parameters.lfr_per_tool
            print('Action 6 completed:')
            if is_fractured in ['no', 'n']:
                print('    HER = %.2f, h/df = %.2f, t/t0 = %.2f.' % (her, h_df, t_t0))
            print('    Overall LFR = %.2f, LFR using R%d = %.2f.' % (overall_lfr, R, lfr_per_tool))
